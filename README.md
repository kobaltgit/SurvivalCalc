...
### 5. 🗺️ Импорт планируемых GPX-маршрутов и нативная интеграция
- **Нативная ассоциация файлов в Android:** автоматическое открытие `.gpx` файлов напрямую из любых приложений (Telegram, WhatsApp, Проводник, Загрузки, Почта) или через системное меню «Поделиться».
...
### 10. 📲 Офлайн-синхронизация по QR-коду, библиотека походов и Ключ похода
...
- **Библиотека «Мои походы»:** сохранение походов, дублирование, шаблоны и системные пресеты.
- **Экспорт постоянного «Ключа похода»:** сохранение зашифрованной ссылки в буфер обмена или в текстовый файл `.txt` для защиты данных от очистки кэша браузера на ПК.
...
- **Два официальных формата туристской отчетности:**
  ...
  2. 📗 **Итоговый технический отчет о походе:**
     ...
     - Векторная схема нитки маршрута с двухцветным компасом, путевыми точками и высотным профилем.
...
### 12. 🩺 Встроенная офлайн «База знаний & Первая помощь» (Wilderness First Aid)
- **15 структурированных офлайн-статей с таблицами и формулами** в 8 категориях: Руководство, Физиология, Раскладка, Снаряжение, GPS/Карты, МКК/ФСТР, Первая помощь и медицина, FAQ.
- **Полевой медицинский справочник:**
  - Протоколы неотложной помощи: гипогликемия («Правило 15»), анафилаксия, приступ астмы, синдром «острого живота», сердечно-легочная реанимация (СЛР 30:2).
  - Травматология: правила наложения турникетов/жгутов, иммобилизация и шинирование из треккинговых палок.
  - Экстремальная медицина: горная болезнь (ОГБ, ОВЛ, ОВМ), гипотермия (протокол «кокон»), укусы гадюки и клещей.
  - Сводный справочник аптечки с показаниями и дозировками.
- **Мгновенный полнотекстовый поиск** по симптомам, препаратам, терминам и формулам.

---

### 13. 🌙 Тактический интерфейс (Outdoor UI)
...
flutter test
```
*(Все 79 unit- и widget-тестов должны пройти со статусом `All tests passed!`)*
...
- **Покрытие тестами:** 79 unit- и widget-тестов покрывают:
  ...
  - Генерацию отчетов МКК в Markdown, чистый экспорт HTML и структуру PDF.
  - Рендеринг всех 15 статей офлайн Базы знаний и компонентов первой помощи.
...
Правила сообщества и разработка:
- 👥 **[Руководство для контрибьюторов (CONTRIBUTING.md)](CONTRIBUTING.md)** — архитектура, локальный запуск, тесты и стандарты PR.
- 🤝 **[Кодекс поведения сообщества (CODE_OF_CONDUCT.md)](CODE_OF_CONDUCT.md)** — нормы конструктивного и уважительного общения.

---

## 🛡️ Свобода платформы, безопасность и приватность
...
- 🛡️ **[Политика безопасности (SECURITY.md)](SECURITY.md)** — регламент ответственного сообщения об уязвимостях и принципы безопасной разработки.
- 🔒 **[Политика конфиденциальности (PRIVACY.md)](PRIVACY.md)** — гарантия 100% отсутствия трекеров, рекламы и сетевой слежки.
